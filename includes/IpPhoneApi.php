...
class IpPhoneApi {
    ...
    public static function keyPress($ip,$url,$logger)
    {
        ...
        curl_setopt($ch, CURLOPT_USERPWD, 'sloanma' . ":" . '$l0whanD58');  // NIPT
        curl_setopt($ch, CURLOPT_POSTFIELDS, $xml);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $output = curl_exec($ch);
        curl_close($ch);

        $logger->logInfo('CURL Output',$output);

        return $output;
    }

    public static function dial($ip,$pattern,$logger)
    {
    ...
    public static function checkWeb($ip,$logger)
    {
